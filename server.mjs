...
function staticBrainSnapshot() {
  return {
    ...
    assignments: [
      ...
      { id: "TIG-001", owner: "Tigra", title: "Cock Dracula / Everyday Vampire launch plan", status: "ASSIGNED", evidence: "3 series, 5 hooks, 7-day schedule, 10 replies, outreach, metrics" },
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname.startsWith("/api/systems/") && req.method === "PATCH") {
      ...
      if (!sys) return json(res, 404, { error: "system not found" });
      const before = sys.status;
      if (body.status) sys.status = body.status;
      state.updatedBy = body.updatedBy || "operator";
      pushEvent(state, "system.update", `${sys.name}: status ${before} → ${sys.status}`, {
        systemId: id,
        by: state.updatedBy,
      });
      return json(res, 200, saveState(state));
    ...
    if (url.pathname.startsWith("/api/sister-repos/") && req.method === "PATCH") {
      ...
      state.updatedBy = body.updatedBy || "operator";
      pushEvent(state, "sister-repo.update", `${repo.name}: status ${before} → ${repo.status}`, {
        repo: name,
        by: state.updatedBy,
      });
      return json(res, 200, saveState(state));
    ...
    if (url.pathname === "/api/sweep" && req.method === "POST") {
      const body = await readBody(req);
      const state = loadState();
      const by = body.updatedBy || body.by || "operator";
      const crewLine = (state.crew || []).map((c) => `${c.name}:${c.presence}`).join(", ") || "none";
      const missionLine = (state.missions || []).map((m) => `${m.id}:${m.status}`).join(", ") || "none";
      const systemLine = (state.systems || []).map((s) => `${s.name}:${s.status}`).join(", ") || "none";
      const repoLine = (state.sisterRepos || []).map((r) => `${r.name}:${r.status}`).join(", ") || "none";
      const store = loadHandoffs();
      const handoffLine = (store.handoffs || []).slice(0, 5).map((h) => `${h.assignmentId}:${h.status}`).join(", ") || "none";
      pushEvent(state, "sweep.crew", `Crew board: ${crewLine}`, { by });
      pushEvent(state, "sweep.mission", `Mission board: ${missionLine}`, { by });
      pushEvent(state, "sweep.system", `System board: ${systemLine}`, { by });
      pushEvent(state, "sweep.sister-repo", `Sister repos: ${repoLine}`, { by });
      pushEvent(state, "sweep.handoff", `Recent handoffs: ${handoffLine}`, { by });
      pushEvent(state, "sweep", "Front-to-back sweep complete", { by });
      state.updatedBy = by;
      return json(res, 200, saveState(state));
    }
