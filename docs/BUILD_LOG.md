...
## 2026-09-07 — Mission title quick-edit

**Shipped**
- Missions panel: **Title** button per row (alongside Evidence). Prompt prefilled with current title; PATCH `/api/missions/:id` with `{ title }` and activity log already records title updates server-side.
- Layout: Title + Evidence in `.mission-edit-btns` under status cycle (mobile-friendly full-width stack).
- Build stamp `2026-09-07-c`. Cache-bust on app.js / operator-extra.js.
- Files: `public/app.js`, `public/index.html`, `public/styles.css`, `docs/BUILD_LOG.md`.
...
## 2026-09-07 — Real escapeHtml fix

**Shipped**
- Fixed `escapeHtml` for real: previous “fix” still replaced characters with themselves (entities had been lost). Now encodes `&`, `<`, `>`, `"` correctly.
- Build stamp `2026-09-07-b`. Cache-bust on app.js / operator-extra.js.
...
## 2026-09-07 — Mission evidence quick-edit

**Shipped**
- Missions panel: **Evidence** button per row. Prompt prefilled with current evidence; PATCH `/api/missions/:id` with `{ evidence }` and logs activity.
- Layout: status cycle buttons + Evidence under `.mission-actions` (mobile-friendly stack).
- Build stamp `2026-09-07-a`. Cache-bust on app.js / operator-extra.js.
- Files: `public/app.js`, `public/index.html`, `public/styles.css`, `docs/BUILD_LOG.md`.
- No Brain write. No secrets. No Amber/Allie merge. ART / CD-001 parked.
