...
function renderMissions(missions) {
  ...
  el.innerHTML = list.map((m) => `
    <div class="row mission-row" data-id="${escapeHtml(m.id)}">
      ...
      <div class="mission-actions">
        ...
        <div class="btn-group mission-edit-btns">
          <button type="button" class="btn sm title-btn" data-title="${escapeHtml(m.title || "")}">Title</button>
  ...
  el.querySelectorAll(".title-btn").forEach((btn) => {
    btn.addEventListener("click", async () => {
      const row = btn.closest(".mission-row");
      const id = row.dataset.id;
      const current = btn.dataset.title || "";
      const next = prompt(`Title for ${id}`, current);
      if (next === null) return;
      const title = next.trim();
      if (!title || title === current) return;
      btn.disabled = true;
      try {
        await api(`/api/missions/${encodeURIComponent(id)}`, { method: "PATCH", body: JSON.stringify({ title, updatedBy: "operator-ui" }) });
        await refresh();
      } catch (err) {
